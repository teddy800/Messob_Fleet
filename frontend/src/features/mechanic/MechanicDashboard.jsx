  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <span className="text-xs font-bold uppercase tracking-widest text-gray-400">{records.length} records</span>
      </div>
            ...
            <TableHeader className="bg-gray-50">
              <TableRow>
                {["Date", "Vehicle", "Service Type", "Mechanic", "Cost", "Status", "Action"].map((h) => (
                  <TableHead key={h} className={`font-bold text-xs uppercase tracking-widest whitespace-nowrap ${h === "Action" ? "text-right" : ""}`}>{h}</TableHead>
                ))}
              </TableRow>
            </TableHeader>
            <TableBody>
              {records.map((r) => (
                <TableRow key={r.id} className="hover:bg-gray-50 transition-colors">
                  ...
                  <TableCell className="font-bold text-sm whitespace-nowrap">{Array.isArray(r.vehicle_id) ? r.vehicle_id[1] : "—"}</TableCell>
                  <TableCell className="text-sm text-gray-700 whitespace-nowrap">{r.service_type}</TableCell>
                  <TableCell className="text-sm text-gray-600 whitespace-nowrap">{Array.isArray(r.mechanic_id) ? r.mechanic_id[1] : "—"}</TableCell>
                  <TableCell className="text-sm font-bold text-gray-800 whitespace-nowrap">{r.cost} ETB</TableCell>
                  <TableCell>
                      ...
                      {Object.entries(statusConfig).map(([key, cfg]) => (
                        <button key={key} onClick={() => handleStatusChange(r.id, key)}
                          className={`text-[10px] font-black uppercase tracking-widest px-2 py-1 rounded-full border transition-all ${
                            r.vehicle_state === key ? cfg.cls + " scale-105 shadow-sm" : "bg-white text-gray-300 border-gray-200 hover:border-gray-400 hover:text-gray-500"
                          }`}>
                          {cfg.label}
                        </button>
                      ))}
              ...
              {records.length === 0 && (
                <TableRow><TableCell colSpan={7} className="text-center text-gray-400 py-8">No maintenance records found.</TableCell></TableRow>
              )}
            </TableBody>
          </Table>
        </div>
      ...
      <Dialog open={viewOpen} onOpenChange={setViewOpen}>
        <DialogContent className="max-w-md p-6">
          <DialogHeader>
            <DialogTitle className="text-brand-blue font-black">Maintenance Record #{selected?.id}</DialogTitle>
          </DialogHeader>
          {selected && (
